...
var GitVersion = 'edcb8b033e6b6c74e7164bf7a5cea6f5b1d35714';
var DocBranch = "1.7/master";
var OtherDocVersion = "1.8-dev";

// set the google CSE key based on the docversion
if (DocVersion.indexOf('-dev') == -1) {
	var GoogleCSE = "005189418096069957794:_6nu-kh1y5u";
	var OtherDocSite = "http://dev-fueldocs.ycnets.com/";
	var FuelLink = "http://fuelphp.com/docs/";
	var GAACC = "UA-2031786-8";
} else {
	var GoogleCSE = "005189418096069957794:8unehy5hih4";
	var OtherDocSite = "http://fueldocs.ycnets.com/";
	var FuelLink = "http://fuelphp.com/dev-docs/";
	var GAACC = "UA-2031786-9";
}
...
function method_list($methods)
{
	...
	$('footer').append('<div id="methods"><a href="#" id="methods_handle">方法列表</a><div id="method_list">'+list.join('')+'</div></div>');
...
panes.core = {
	"類別": {
		...
		"Cache": {
			...
			"用法":	"classes/cache/usage.html",
			"進階":	"classes/cache/advanced.html"
		},
		...
		"Database": {
			...
			"建立查詢":	{
				...
				"Delete 建立器":	"classes/database/qb_delete.html",
				"Where 建立器":	"classes/database/qb_where.html"
			}
		},
		...
		"Request": {
			"URI 請求":		"classes/request/request.html",
...
panes.auth = {
	"Auth 套件": {
		...
		"範例": {
			"*Auth 控制器": "packages/auth/examples/auth.html",
			"*Opauth 控制器": "packages/auth/examples/opauth.html"
		}
	}
};
...
panes.orm = {
	"Orm 套件": {
		...
		"模型": {
			"虛刪除":			"packages/orm/model/soft.html",
			"時序／修訂版本":	"packages/orm/model/temporal.html",
			"*巢狀集合":		"packages/orm/model/nestedset.html"
		},
	...
	$.each(pane_contents, function(subject, subject_contents){
		...
		$.each(subject_contents, function(item, item_contents){
			...
			else
			{
				...
				{
					item = item.substr(1);
					class_arr.push('uncompleted');
				}
				if (active_path.indexOf(item_contents, active_path.length - item_contents.length) != -1)
				{
					class_arr.push('active');
				}
				if ($.isArray(class_arr))
				{
					classes = ' class="'+implode(' ', class_arr)+'"';
				}
				html = html + '<li><a '+classes+' href="'+path+item_contents+'">'+item+'</a></li>';
			}
...
$("document").ready(function(){
	...
	$('footer p')
		.append('[ <a href="'+FuelLink+pagepath+'">原文</a> | <a href="https://github.com/fuel/docs/commits/'+DocBranch+'/'+pagepath+'">歷史</a> | <a href="'+OtherDocSite+pagepath+'">'+OtherDocVersion+'</a> ]')
		.append(' <a href="https://github.com/fuel/docs/commit/'+GitVersion+'"><code>'+GitVersion.substr(0, 7)+'</code></a> <a class="lance" href="http://blog.ycnets.com">@LanceHe</a>');
